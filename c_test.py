...
def required_C_and_D(secret, error, l_exp):
    # assuming alpha = 1, since both error and secret are from the same distribution
    tau = ((pow(np.inner(error, error), 2) + pow(np.inner(secret[-k_lat:], secret[-k_lat:]), 2)) / (m + k_lat)) * pow(l_exp, 2)
...
def D_of_advantage(l_exp):
    ...
    print(f"LATEST ESTIMATE: {D_est}")
    return ceil(D_est)

def C_of_advantage(D):
    phi_fp = dist.CDF_INV((1 - mi / (2 * pow(2, k_enum * dist.entropy) * pow(p, k_fft))))
    C_estimate = phi_fp * sqrt(1/2 * D)
    print(f"LATEST ESTIMATE: {C_estimate}")
    return C_estimate
...
def dual_attack_test(A, L, y_ffts, y_enums):
    ...
    D_est, C = required_C_and_D(s, e, l_exp)
    # Line 4:
...
if __name__ == '__main__':
    ...
    folder_path = "./c_test/" + f"{current_date}_{n}_{m}/"
