...
def required_number_of_samples(secret, error, l_exp):
    ...
    print(f"D_eq: {D_eq}")
    print(f"D_round: {D_round}")
    print(f"D_arg: {D_arg}")
    print(f"D_fpfn: {D_fpfn}")
    print(f"D (Concrete): {D_estimates}")
    print(f"C (Concrete): {C_estimates}")
    logging.info(f"D (Concrete): {D_estimates}")

    
def parameters_of_advantage(l_exp):
    D_estimates = []
    C_estimates = []
