...
from math import sqrt, ceil, pi, sin, cos, exp, log, log2, comb, prod, erf
...
def asympthotic_D(l_exp):
    ...
    Parameters: l_exp (float): Average length of short vector returned by the sampling algorithm
    ...
    D_fpfn_tilde = (k_enum * dist.entropy + k_fft * log(p) + log(1./0.001))          
    return ceil(D_eq_tilde * D_round_tilde * D_fpfn_tilde)
...
def dual_attack_test(A, L, y_ffts, y_enums):
    ...
    _max = mod_switch_distinguisher(y_enums, y_ffts, q, p, k_fft, L, np.array(s[:k_enum]), b)
...
if __name__ == '__main__':
    ...
    folder_path = "./dist_test/" + f"{current_date}_{n}_{m}/"
    ...
    num_of_trials = 500 # how many times the test should be repreated
    ...
    for i in range (num_of_trials):
        s_klat, error, f_max = dual_attack_test(A, L, y_ffts, y_enums)     
        s_klats.append(s_klat)
        errors.append(error)
        f_maxs.append(f_max)
    ...
    # Lemma 5.3.
    tau_squared = pow(l_exp, 2) * (s_avg + e_avg) / (m + k_lat) #alpha = 1, since secret and error distributions are the same
    E_eps_eq = exp(-2 * pow((math.pi / q), 2) * tau_squared)
    E_eps_sq_eq = 2 * exp(-8 * pow((math.pi / q), 2) * tau_squared)
    # Lemma 5.4.
    E_eps_round = prod((sin(pi * s / p) / (pi * s / p)) ** (k_fft * dist.probability_distribution[s]) for s in dist.probability_distribution.keys() if pow(s, 1, p) != 0)
    # Lemma 5.5.
    ...
    logging.info("Ks_test: {ks_test}")
    print(f"shapiro_test: {shapiro_test}")
    logging.info("shapiro_test: {shapiro_test}")
    ...
    plt.plot(f_maxs, empirical_cdf_values, marker='.', linestyle='-', label='Empirical CDF')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"new_values_{n}_{k_enum}_{k_fft}.png", dpi=300)
    plt.show()
