"""
This script contains the code for the experiments discussed in Chapter 5, Section 4.
It examines the bound on the cutoff value C for the dual attack proposed by MATZOV in Theorem 4.3.2.
"""
import numpy as np
import logging
import os
...
def D_of_advantage(ell):
    ...
    print(f"D_eq_tilde: {D_eq_tilde}")
    print(f"hi_fp: {phi_fp}")
    print(f"phi_fn: {phi_fn}")
    print(f"D_fpfn_tilde: {D_fpfn_tilde}")
    print(f"D_round_tilde: {D_round_tilde}")
    logging.info(f"D_este: {D_est}")
    logging.info(f"D_eq_tilde: {D_eq_tilde}")
    logging.info(f"hi_fp: {phi_fp}")
    logging.info(f"phi_fn: {phi_fn}")
    logging.info(f"D_fpfn_tilde: {D_fpfn_tilde}")
    logging.info(f"D_round_tilde: {D_round_tilde}")
    logging.info(f"D_este: {D_est}")
...
def C_of_advantage(D):
    """
    Calculate the approperiate value for C according to Theorem 4.3.2

    Parameters:
    - D (int): list of short vectors

    Returns: 
    - C (float)
    """
    phi_fp = dist.CDF_INV((1 - (mi / (2 * pow(2, k_enum * dist.entropy) * pow(p, k_fft)))))
    print(f"phi_fp for C: {phi_fp }")
    logging.info(f"phi_fp for C: {phi_fp }")
    print(f"Based on: {D}")
    C_estimate = np.real(phi_fp) * sqrt( (1/2) * D)
...
def parse_parametrs():
    ...
    parser.add_argument('--Beta2', type=int, default=40)
...
if __name__ == '__main__':
    ...
    folder_path = "./c_for_advantage/" + f"{n}_{m}/" + f"{current_date}_{k_enum}_{k_fft}_{p}/"
    ...
    D_of_advantage(ell)
    C = C_of_advantage(D)   
    print(f"cutoff value C: {C}")

    num_of_trials = 10 # how many times the test should be repreated
